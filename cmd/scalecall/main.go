// Command scalecall drives agenthost.RunScaleCheckpoint from argv, for
// Experimento 1-bis Q1+Q2's corridas (~/exo/build_prompt_YEYO_Q1Q2.md) —
// same throwaway-experiment-tooling pattern as cmd/checkpointcall (exp1k/
// exp1k2) and cmd/rawcall (exp1h). Prints one JSON object per corrida to
// stdout. Not part of exo's normal build/install.
package main
...
func main() {
	if len(os.Args) != 7 {
		fmt.Fprintln(os.Stderr, "usage: scalecall <n> <seed> <task-label> <expected yes|no> <user-message> <project-root-abs-path>")
		os.Exit(1)
	}
	n, err := strconv.Atoi(os.Args[1])
	if err != nil || n < 1 {
		fmt.Fprintf(os.Stderr, "invalid n %q: must be a positive integer\n", os.Args[1])
		os.Exit(1)
	}
	...
	// Same reasoning as cmd/checkpointcall: Phase 2's normal tools resolve
	// relative task paths against the process cwd.
	...
	result, runErr := agenthost.RunScaleCheckpoint(context.Background(), message, &trace, n, seed)
