// Command q6call drives agenthost.RunQ6Checkpoint from argv, for
// Experimento 1-bis Q6's corridas — same throwaway-experiment-tooling
// pattern as cmd/q4call (Q4), cmd/q3ccall (Q3C), cmd/q3bcall (Q3B),
// cmd/q3call (Q3), cmd/scalecall (Q1+Q2), cmd/checkpointcall (exp1k/exp1k2),
// and cmd/rawcall (exp1h). Prints one JSON object per corrida to stdout. Not
...
type logEntry struct {
	...
	Positions     map[string]int                 `json:"positions"` // split-large-file / preserve-public-api / update-package-docs
...
func main() {
	if len(os.Args) != 5 {
		fmt.Fprintln(os.Stderr, "usage: q6call <seed> <task-label> <user-message> <project-root-abs-path>")
		os.Exit(1)
	}
	...
	result, runErr := agenthost.RunQ6Checkpoint(context.Background(), message, &trace, seed)
